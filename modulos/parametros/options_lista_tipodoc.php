<?php
   // $latabla='carto_fa_ecuador_parroquial';
	...
	$sql = "SELECT id, codigo_doc, tipo   FROM tbli_esq_plant_formunico_tipodoc where ventanilla_doc=1 order by id ;";
...
if ($_REQUEST["t"] == "combo") {
	...
	echo '<?xml version="1.0" encoding="utf-8"?>'.'<complete>';
      for($im=0;$im<$numerdatos;$im++)
	  {
	    echo "<option value='".pg_fetch_result($resulprovs,$im,0)."'>".pg_fetch_result($resulprovs,$im,1).": ".pg_fetch_result($resulprovs,$im,2)."</option>";
	  }
	echo	"</complete>";
	/////////////////////////////////////////////
