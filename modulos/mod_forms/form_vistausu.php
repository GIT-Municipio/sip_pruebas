...
$varcodactual=pg_fetch_result($myresuldatusu,0,'cod_tramite_tempo');
...
<table width="100%" border="0" bgcolor="#FFFFFF">
  <tr>
    ...
    <td width="164" align="center"><table width="200" border="0">
        <tr>
          <td align="center"><a href="form_vimpresion.php?rp=<?php echo $_GET['rp']; ?>&varclaveuntramusu=<?php echo $_GET['varclaveuntramusu']; ?>"><img src="imgs/vistaprevia.png" width="40" height="40" border="0" /></a></td>
        </tr>
        <tr>
    ...
    for($grup=0;$grup<pg_num_rows($resulvgrups);$grup++)
	{
		...
		echo '<table width="100%" border="1" cellpadding="0" cellspacing="0">';
		 for($items=0;$items<pg_num_rows($resulvcamps);$items++)
	     {
			...
			{
				/*echo 'app_tipo_mapaubic_vista.php?ponref_plantilla='.pg_fetch_result($resulvcamps,$items,"ref_plantilla").'&varitabcmpid='.pg_fetch_result($resulvcamps,$items,"campo_creado").'&varclaveuntramusu='.$varclaveunictramitusu;
				*/
...
echo "</tr>";
  for($i=0;$i<pg_num_rows($consul);$i++)
	    ...
	    $rescampocombo = pg_query($conn, $sqlcampscombo);
			...
			{
				if(pg_fetch_result($resulvcamps,$items,"campo_requerido")==1)
    		     echo  "<td align='center'  height='25'><input name='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' type='text' id='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' value='".pg_fetch_result($myresuldatusu,0,pg_fetch_result($resulvcamps,$items,"campo_creado"))."' style='width: 90%' readonly required  title='Este campo es requerido'  /></td>";
				 else
				  echo  "<td align='center'  height='25'><input name='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' type='text' id='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' value='".pg_fetch_result($myresuldatusu,0,pg_fetch_result($resulvcamps,$items,"campo_creado"))."' style='width: 90%' readonly /></td>";
