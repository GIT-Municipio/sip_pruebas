...
<table width="100%" border="0" bgcolor="#FFFFFF">
  <tr>
    ...
    <td width="164" align="center"><table width="200" border="0">
        <tr>
          <td align="center"><a href="form_vimpresiontecn.php?rp=<?php echo $_GET['rp']; ?>&varclaveuntramusu=<?php echo $_GET['varclaveuntramusu']; ?>"><img src="imgs/vistaprevia.png" width="40" height="40" border="0" /></a></td>
        </tr>
        <tr>
          <td align="center">Vista Previa</td>
        </tr>
    </table></td>
  </tr>
  <tr>
    ...
    <?php
	  $sqlgrups = "select id,titulo_grupo,orden_grupo,publico  FROM tbli_esq_plant_form_plantilla_grupo where ref_plantilla='".$_GET['rp']."' and activo=1 order by orden_grupo";
	 $resulvgrups = pg_query($conn, $sqlgrups);
	 
	 echo '<form id="formCiudadano" name="formCiudadano" method="post" action="guardarinfo_plantecn.php">';
    ...
    for($grup=0;$grup<pg_num_rows($resulvgrups);$grup++)
	{
		...
		echo '<table width="100%" border="1" cellpadding="0" cellspacing="0">';
		 for($items=0;$items<pg_num_rows($resulvcamps);$items++)
	     {
			...
			{
				   echo '<iframe name="map'.pg_fetch_result($resulvcamps,$items,"nombre_tablacmp").'" id="map'.pg_fetch_result($resulvcamps,$items,"nombre_tablacmp").'" frameborder="0"  width="100%" height="400"  src="app_tipo_mapaubic_vista.php?ponref_plantilla='.pg_fetch_result($resulvcamps,$items,"ref_plantilla").'&varitabcmpid='.pg_fetch_result($resulvcamps,$items,"campo_creado").'&varclaveuntramusu='.$varclaveunictramitusu.' "></iframe>';
			}
	    ...
	    $rescampocombo = pg_query($conn, $sqlcampscombo);
			...
			{
				if(pg_fetch_result($resulvcamps,$items,"campo_requerido")==1)
    		     echo  "<td align='center'  height='25'><input name='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' type='text' id='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' value='".pg_fetch_result($myresuldatusu,0,pg_fetch_result($resulvcamps,$items,"campo_creado"))."' style='width: 90%' required  title='Este campo es requerido'  /></td>";
				 else
				  echo  "<td align='center'  height='25'><input name='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' type='text' id='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' value='".pg_fetch_result($myresuldatusu,0,pg_fetch_result($resulvcamps,$items,"campo_creado"))."' style='width: 90%'  /></td>";
	...
	echo '<input type="submit" name="btnenviar" id="btnenviar" value="" style="background-image:url(imgs/form_btnguardar.png); color:#fff;width:202px;height:40px; font-size:14px" />';
	echo '</form>';
