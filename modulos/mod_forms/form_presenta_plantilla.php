...
<table width="100%" border="0" bgcolor="#FFFFFF">
  ...
  <tr>
    ...
    <?php
	  $sqlgrups = "select id,titulo_grupo,orden_grupo,titulo,mostrar_titulo  FROM tbli_esq_plant_form_plantilla_grupo where ref_plantilla='".$_GET['varidplanty']."' and activo=1 order by orden_grupo";
    ...
    for($grup=0;$grup<pg_num_rows($resulvgrups);$grup++)
	{
		...
		echo '<fieldset><legend>'.pg_fetch_result($resulvgrups,$grup,"titulo_grupo").'</legend>';
		 ...
		 $sqlcamps = "select id,campo_creado,campo_nombre,campo_requerido,ref_tcamp,campo_orden,nombre_tablacmp,ref_plantilla,nombre_combocmp,publico,visible,valorx_defecto  FROM tbli_esq_plant_form_plantilla_campos where ref_plantilla='".$_GET['varidplanty']."' and ref_grupo='".pg_fetch_result($resulvgrups,$grup,"id")."' and campo_activo=1 order by campo_orden; ";
		...
		echo '<table width="100%" border="1" cellpadding="0" cellspacing="0">';
		 for($items=0;$items<pg_num_rows($resulvcamps);$items++)
	     {
			...
			{
				   echo '<td colspan="2" align="center"><table><tr><td  align="right"><a href="form_imagen.php?varidplantilla='.pg_fetch_result($resulvcamps,$items,"ref_plantilla").'&varidenvcmp='.pg_fetch_result($resulvcamps,$items,"id").'&varclaveuntramusu=0">Editar Imagen<img src="imgs/combo_camp.png" width="18" height="18" border="0" /></a></td></tr><tr><td><div name="text_'.pg_fetch_result($resulvcamps,$items,"campo_creado").'" id="text_'.pg_fetch_result($resulvcamps,$items,"campo_creado").'" ><img src="../../../sip_bodega/archformularios/'.pg_fetch_result($resulvcamps,$items,"valorx_defecto").'" width="500" height="300" /></div></td></tr></table></td>';
				      
			}
			else  if(pg_fetch_result($resulvcamps,$items,"ref_tcamp")==11)
			{
				   echo '<td colspan="2" align="center"><table><tr><td  align="right"><a href="guarda_textopredefinido.php?varidplantilla='.pg_fetch_result($resulvcamps,$items,"ref_plantilla").'&varidenvcmp='.pg_fetch_result($resulvcamps,$items,"id").'&varclaveuntramusu=0">Editar Texto<img src="imgs/combo_camp.png" width="18" height="18" border="0" /></a></td></tr><tr><td><div name="text_'.pg_fetch_result($resulvcamps,$items,"campo_creado").'" id="text_'.pg_fetch_result($resulvcamps,$items,"campo_creado").'" >'.pg_fetch_result($resulvcamps,$items,"valorx_defecto").'</div></td></tr></table></td>';
			...
			if(pg_fetch_result($resulvcamps,$items,"campo_requerido")==1)
			{
				 if(pg_fetch_result($resulvcamps,$items,"publico")==1)
				 {
					  ...
					  if(pg_fetch_result($resulvcamps,$items,"visible")==0)
    		 			echo '<td  bgcolor="#E2E2E2" width="40%"><b>'.pg_fetch_result($resulvcamps,$items,"campo_nombre").':</b><b><font color="#FF0000" size="2" face="Arial, Gadget, sans-serif"> (*)</font><font color="#2ca61b" size="2" face="Arial, Gadget, sans-serif"> (P)</font><font color="#00F" size="2" face="Arial, Gadget, sans-serif"> (O)</font></b></td>';
			 else
				  ...
				  {
					  if(pg_fetch_result($resulvcamps,$items,"visible")==0)
					echo '<td  bgcolor="#E2E2E2" width="40%"><b>'.pg_fetch_result($resulvcamps,$items,"campo_nombre").':</b><font color="#FF0000" size="2" face="Arial, Gadget, sans-serif"> (*)</font><font color="#00F" size="2" face="Arial, Gadget, sans-serif"> (O)</font></td>';
			else
			 ...
			 else
			 {
				 ...
				 {
					 if(pg_fetch_result($resulvcamps,$items,"visible")==0)
			  echo '<td  bgcolor="#E2E2E2" width="40%"><b>'.pg_fetch_result($resulvcamps,$items,"campo_nombre").':</b><b><font color="#2ca61b" size="2" face="Arial, Gadget, sans-serif"> (P)</font><font color="#00F" size="2" face="Arial, Gadget, sans-serif"> (O)</font></b></td>';
			  else
			  ...
			  {
				   if(pg_fetch_result($resulvcamps,$items,"visible")==0)
			  echo '<td  bgcolor="#E2E2E2" width="40%"><b>'.pg_fetch_result($resulvcamps,$items,"campo_nombre").':</b><font color="#00F" size="2" face="Arial, Gadget, sans-serif"> (O)</font></td>';
			  else
	    ...
	    $rescampocombo = pg_query($conn, $sqlcampscombo);
				...
				echo "<td  align='center' ><table width='100%' border='0'><tr><td width='898'  align='center'><select name='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' id='".pg_fetch_result($resulvcamps,$items,"campo_creado")."' style='width: 90%' >";
				for($icomb=0;$icomb<pg_num_rows($rescampocombo);$icomb++)
					{
  					...
  					echo		"</select></td><td width='40'><a href='app_tipo_combo.php?pontabla=".pg_fetch_result($resulvcamps,$items,"nombre_combocmp")."&varitabcmpid=".pg_fetch_result($resulvcamps,$items,"id")."&varclaveuntramusu=0'><img src='imgs/combo_camp.png' width='18' height='18' border='0' /></a></td></tr></table></td>";
