...
$sql="SELECT  ROW_NUMBER () OVER (ORDER BY gparam_nom_categoria) as id,gparam_nom_categoria as categoria,count(*) as total_archivos FROM public.tbl_archivos_scanimgs where est_eliminado=0  and date(fecha)=date('".$_GET['enviofechacons']."')   group by gparam_nom_categoria order by gparam_nom_categoria;";
