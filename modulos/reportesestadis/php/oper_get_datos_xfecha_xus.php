...
$sql="SELECT  ROW_NUMBER () OVER (ORDER BY gusu_respons_edit) as id,gusu_respons_edit as usuario,count(*) as total_archivos FROM public.tbl_archivos_scanimgs where est_eliminado=0  and date(fecha)=date('".$_GET['enviofechacons']."')  group by gusu_respons_edit order by gusu_respons_edit;";
