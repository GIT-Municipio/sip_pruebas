...
function deleteSingleNode($id){
		$d_sql = "Delete from ".$_REQUEST[mitabla]." where id=".$id;
		// $resDel = mysql_query($d_sql);
		$resDel = pg_query($conn,$d_sql);
	}
//deletes all child nodes of the item
function deleteBranch($pid){
	$s_sql = "Select id,item_orden from ".$_REQUEST[mitabla]." where ".$_REQUEST[ref_parent_padre]."=$pid";;
...
if(isset($_REQUEST[$_REQUEST[ids]."_!nativeeditor_status"]) && $_REQUEST[$_REQUEST[ids]."_!nativeeditor_status"]=="inserted"){
	...
	$sql_uorders = "UPDATE ".$_REQUEST[mitabla]." SET item_orden=item_orden+1 WHERE ".$_REQUEST[ref_parent_padre]."=".$_REQUEST[$_REQUEST[ids]."_tr_pid"]." AND item_orden>=".$_REQUEST[$_REQUEST[ids]."_tr_order"]." and id!=".$newId;
	...
	$sql_getoldparent = "Select ".$_REQUEST[ref_parent_padre].",item_orden from ".$_REQUEST[mitabla]." where id=".$_REQUEST[$_REQUEST[ids]."_tr_id"];
	...
	//update node info 
	$sql = 	"Update ".$_REQUEST[mitabla]." set ".$_REQUEST[minombredato]." = '".addslashes($_REQUEST[$_REQUEST[ids]."_tr_text"])."',".$_REQUEST[ref_parent_padre]." = ".$_REQUEST[$_REQUEST[ids]."_tr_pid"].",item_orden = ".$_REQUEST[$_REQUEST[ids]."_tr_order"]." where id=".$_REQUEST[$_REQUEST[ids]."_tr_id"];
	//update nodes order on old node level (after drag-n-drop node level can be changed)
	$sql_uorders_old = "UPDATE ".$_REQUEST[mitabla]." SET item_orden=item_orden-1 WHERE ".$_REQUEST[ref_parent_padre]."=".$old_values[0]." AND item_orden>".$old_values[1]." and id<>".$_REQUEST[$_REQUEST[ids]."_tr_id"];
	//update nodes order on current node level
	$sql_uorders_new = "UPDATE ".$_REQUEST[mitabla]." SET item_orden=item_orden+1 WHERE ".$_REQUEST[ref_parent_padre]."=".$_REQUEST[$_REQUEST[ids]."_tr_pid"]." AND item_orden>=".$_REQUEST[$_REQUEST[ids]."_tr_order"]." and id<>".$_REQUEST[$_REQUEST[ids]."_tr_id"];
		//$res = mysql_query($sql);
		$res = pg_query($conn,$sql);
