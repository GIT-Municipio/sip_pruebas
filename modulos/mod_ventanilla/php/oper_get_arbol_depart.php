...
//print tree XML based on parent_id (function calls itself to go through the nested levels)
	function getLevelFromDB($parent_id){
		...
		//if($_REQUEST[princonsulfre]!="")
	 $sql = "SELECT  ".$_REQUEST[elidprincipal].", ".$_REQUEST[minombredato]." FROM ".$_REQUEST[mitabla]." WHERE ".$_REQUEST[ref_parent_padre]."=$parent_id and estado_depart is true  ORDER BY ".$_REQUEST[elidprincipal];
