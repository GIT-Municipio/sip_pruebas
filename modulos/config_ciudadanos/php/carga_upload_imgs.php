...
if (@$_REQUEST["mode"] == "html5" || @$_REQUEST["mode"] == "flash") {
	$filename = $_FILES["file"]["name"];
	 move_uploaded_file($_FILES["file"]["tmp_name"],"../../galeriaimgs/".$filename);
...
if (@$_REQUEST["mode"] == "html4") {
	...
	if (@$_REQUEST["action"] == "cancel") {
		...
		$filename = $_FILES["file"]["name"];
		 move_uploaded_file($_FILES["file"]["tmp_name"], "../../galeriaimgs/".$filename);
		print_r("{state: true, name:'".str_replace("'","\\'",$filename)."', size:".$_FILES["file"]["size"]/*filesize("../../galeriaimgs/".$filename)*/.", extra: {info: 'just a way to send some extra data', param: 'some value here'}}");
	}
}
...
if (@$_REQUEST["mode"] == "sl" && isset($_REQUEST["fileSize"]) && isset($_REQUEST["fileName"]) && isset($_REQUEST["fileKey"])) {
	...
	if (@$p[0] === $_REQUEST["fileKey"]) {
		...
		$temp_name = "../../galeriaimgs/".$p[0]."_data";
		...
		if (@$_REQUEST["action"] != "getUploadStatus") {
			...
			if (file_exists($temp_name)) {
				rename($temp_name, "../../galeriaimgs/".$_REQUEST["fileName"]);
				$state = "true";
			}
